...
describe("parsePartialVersion", () => {
  it("parses full version", () => {
    expect(parsePartialVersion("1.2.3")).toEqual({ major: 1, minor: 2, patch: 3 });
  });
  ...
  it("parses major.minor", () => {
    expect(parsePartialVersion("2.7")).toEqual({ major: 2, minor: 7, patch: 0 });
  });
...
describe("satisfiesRange", () => {
  ...
  it("^1.2.3 matches 1.x.x >= 1.2.3", () => {
    expect(satisfiesRange({ major: 1, minor: 5, patch: 0 }, "^1.2.3")).toBe(true);
    expect(satisfiesRange({ major: 1, minor: 2, patch: 3 }, "^1.2.3")).toBe(true);
    expect(satisfiesRange({ major: 1, minor: 99, patch: 99 }, "^1.2.3")).toBe(true);
  });

  it("^1.2.3 rejects 2.x", () => {
    expect(satisfiesRange({ major: 2, minor: 0, patch: 0 }, "^1.2.3")).toBe(false);
  });

  it("^1.2.3 rejects versions below range", () => {
    expect(satisfiesRange({ major: 1, minor: 2, patch: 2 }, "^1.2.3")).toBe(false);
    expect(satisfiesRange({ major: 1, minor: 1, patch: 0 }, "^1.2.3")).toBe(false);
  });

  it("~1.2.3 matches 1.2.x >= 1.2.3", () => {
    expect(satisfiesRange({ major: 1, minor: 2, patch: 5 }, "~1.2.3")).toBe(true);
    expect(satisfiesRange({ major: 1, minor: 2, patch: 3 }, "~1.2.3")).toBe(true);
  });

  it("~1.2.3 rejects 1.3.x", () => {
    expect(satisfiesRange({ major: 1, minor: 3, patch: 0 }, "~1.2.3")).toBe(false);
  });

  it(">=1 <3 range", () => {
    expect(satisfiesRange({ major: 1, minor: 0, patch: 0 }, ">=1 <3")).toBe(true);
    expect(satisfiesRange({ major: 2, minor: 5, patch: 0 }, ">=1 <3")).toBe(true);
    expect(satisfiesRange({ major: 3, minor: 0, patch: 0 }, ">=1 <3")).toBe(false);
    expect(satisfiesRange({ major: 0, minor: 9, patch: 0 }, ">=1 <3")).toBe(false);
  });

  it("|| operator (either side matches)", () => {
    expect(satisfiesRange({ major: 1, minor: 0, patch: 0 }, "^1.0.0 || ^2.0.0")).toBe(true);
    expect(satisfiesRange({ major: 2, minor: 0, patch: 0 }, "^1.0.0 || ^2.0.0")).toBe(true);
    expect(satisfiesRange({ major: 3, minor: 0, patch: 0 }, "^1.0.0 || ^2.0.0")).toBe(false);
  });

  it("explicit version match", () => {
    expect(satisfiesRange({ major: 1, minor: 2, patch: 3 }, "1.2.3")).toBe(true);
    expect(satisfiesRange({ major: 1, minor: 2, patch: 4 }, "1.2.3")).toBe(false);
  });

  it("^0.0.1 constrains to 0.0.x < 1.0.0", () => {
    expect(satisfiesRange({ major: 0, minor: 0, patch: 1 }, "^0.0.1")).toBe(true);
    expect(satisfiesRange({ major: 0, minor: 5, patch: 0 }, "^0.0.1")).toBe(true);
    expect(satisfiesRange({ major: 1, minor: 0, patch: 0 }, "^0.0.1")).toBe(false);
  });

  it("^0.1.0 constrains to 0.x.x < 1.0.0", () => {
    expect(satisfiesRange({ major: 0, minor: 1, patch: 0 }, "^0.1.0")).toBe(true);
    expect(satisfiesRange({ major: 0, minor: 9, patch: 9 }, "^0.1.0")).toBe(true);
    expect(satisfiesRange({ major: 1, minor: 0, patch: 0 }, "^0.1.0")).toBe(false);
  });
});
